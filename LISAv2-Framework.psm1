...
function Start-LISAv2 {
	...
	PROCESS {
		try {
			$libraryModuleNames = Import-LibrariesModules -WorkingDirectory $PSScriptRoot
			...
			if ($workingDirectory.Length -gt $maxDirLength) {
				...
				$libraryModuleNames | Foreach-Object {Remove-Module -Name $_ -Verbose:$false}
				$null = Import-LibrariesModules -WorkingDirectory $workingDirectory
			}
			...
			if (!(test-path "$WorkingDirectory\Temp")) {
				New-Item -Path "$WorkingDirectory\Temp" -ItemType Directory -Force | Out-Null
			}
			$env:TEMP = "$WorkingDirectory\Temp"
